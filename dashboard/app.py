...
# Menampilkan nama kolom untuk debug
st.write("Nama Kolom dalam DataFrame:", day_df.columns)
...
selected_weather = st.sidebar.selectbox("Pilih Kondisi Cuaca", options=['All'] + list(day_df['weather_cond'].unique()))  # Ganti 'weather_condition' jika perlu
...
if selected_weather != 'All':
    filtered_data = filtered_data[filtered_data['weather_cond'] == selected_weather]  # Ganti 'weather_condition' jika perlu
...
# Visualisasi distribusi penyewaan berdasarkan hari kerja dan hari libur
st.header("Distribusi Penyewaan: Hari Kerja vs Hari Libur")
...
ax.bar(['Hari Kerja', 'Hari Libur'], comparison_df['count']['sum'], color=['lightblue', 'lightcoral'], label='Total Penyewaan')
...
for bar in bars:
    yval = bar.get_height()
    ax.text(bar.get_x() + bar.get_width() / 2, yval + 5000, format(int(yval), ','), ha='center', va='bottom')
...
ax.bar(x, yearly_counts['casual'], width=bar_width, color='skyblue', label='Pengguna Casual')
ax.bar([p + bar_width for p in x], yearly_counts['registered'], width=bar_width, color='lightgreen', label='Pengguna Terdaftar')
